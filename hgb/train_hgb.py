...
def main(args):
    ...
    # Load dataset
    g, adjs, features_list_dict, labels, num_classes, dl, train_nid, val_nid, test_nid, test_nid_full \
        = load_dataset(args)
        ...
        for r in r_list:
            if args.SGA:
                ...
                for k in adjs.keys():
                    ...
                    col_sum = adjs[k[1]+k[0]].sum(dim=1)   ###adjs[k].sum(dim=0)  adjs[k].t()
                ...
                if args.dataset != "Freebase":
                    ...
                    start = time.time()
                    features_list_dict, extra_features_buffer = hg_propagate_sparse_pyg_A(adjs, features_list_dict_cp, tgt_type, args.num_hops, max_length, extra_metapath, threshold_metalen, prop_device, args.enhance, prop_feats=True, echo=True)                    
                    end = time.time()
                    
                    print("time for SGA", end - start)
                    feats = {}
                    feats_extra = {}
                ...
                g = gen_rel_subset_feature_r_ensemble(g, tgt_type, args.num_hops, max_length, r, extra_metapath, echo=True)
                feats = {}
            ...
            if args.label_feats:
                ...
                #meta_adjs = features_list_dict
                prop_tic = datetime.datetime.now()
                num_tgt_nodes = dl.nodes['count'][0]
                if args.dataset == 'Freebase' and args.num_label_hops <= args.num_hops and len(extra_metapath) == 0:
                    meta_adjs = hg_propagate_sparse_pyg_mask(
                                adjs, tgt_type, args.num_label_hops, max_length_label, extra_metapath, prop_feats=False, echo=True, prop_device=prop_device)
                ...
                if args.dataset == 'Freebase':
                    if 0:
                        ...
                        check_acc(label_feats, condition, labels, train_nid, val_nid, test_nid, show_test=False)
                    ...
                    for k, v in tqdm(meta_adjs.items()):
                        if args.dataset != 'Freebase':
                            ...
                            label_feats[k] = (remove_diag(v).to(prop_device) @ label_onehot_g).to('cpu')
                        #label_feats_cache = torch.load(f"/home/lyx/Nars_ensemble/features/label_features/{args.dataset}/seed_{args.seed}_hop_{args.num_label_hops}/r_{r}.pt")
